...
void SkipSimplifiedLayerNormalization_AVX2(const float* input, const float* skip, const float* gamma, float* output, size_t H, float epsilon) {
    ...
    // 1. Compute (input + skip) and accumulate sum of squares
    for (; i + VEC <= H; i += VEC) {
        __m256 vin = _mm256_loadu_ps(input + i);
        __m256 vskip = _mm256_loadu_ps(skip + i);
        __m256 vadd = _mm256_add_ps(vin, vskip);
        _mm256_storeu_ps(output + i, vadd); // Temporarily store (input+skip) in output
    ...
    for (; i + VEC <= H; i += VEC) {
        __m256 vadd = _mm256_loadu_ps(output + i); // (input+skip)
    ...
    for (; i < H; ++i) {
        output[i] = gamma[i] * output[i] * denom;
    }
}
