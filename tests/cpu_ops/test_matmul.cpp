...
int main() {
    ...
    if (!fp) { std::cerr << "Failed to create temp weights file!\n"; return 1; }
    ...
    if (hFile == INVALID_HANDLE_VALUE) { std::cerr << "Failed to open temp weights file!\n"; return 1; }
    HANDLE hMap = CreateFileMappingA(hFile, NULL, PAGE_READONLY, 0, 0, NULL);
    if (!hMap) { std::cerr << "Failed to create file mapping!\n"; CloseHandle(hFile); return 1; }
    float* B_mmap = (float*)MapViewOfFile(hMap, FILE_MAP_READ, 0, 0, K*N*sizeof(float));
    if (!B_mmap) { std::cerr << "Failed to map view of file!\n"; CloseHandle(hMap); CloseHandle(hFile); return 1; }
