...
{
    log_info("=== Qwen3 Decoder Module Test C++ run ===");
    ...
    try
    {
        ...
        Safetensor weights(args.safetensor_path, false);
        weights_timer.stop();
        log_info("Loaded weights from " + args.safetensor_path);

        const auto *input_norm_info = require_tensor_info(weights, "model.layers.0.input_layernorm.weight");
        ...
        const auto *post_attn_norm_info = require_tensor_info(weights, "model.layers.0.post_attention_layernorm.weight");
        const auto *mlp_up_proj_info = require_tensor_info(weights, "model.layers.0.mlp.up_proj.weight");
        const auto *mlp_gate_proj_info = require_tensor_info(weights, "model.layers.0.mlp.gate_proj.weight");
        const auto *mlp_down_proj_info = require_tensor_info(weights, "model.layers.0.mlp.down_proj.weight");
        ...
        if (post_attn_norm_info->shape.at(0) != embed_dim || input_norm_info->shape.at(0) != embed_dim)
        {
            throw std::runtime_error("Norm weight size mismatch with embed_dim");
        }
        ...
        Tensor input_norm_tensor = wrap_tensor(weights, "model.layers.0.input_layernorm.weight", input_norm_info);
        Tensor post_attn_norm_tensor = wrap_tensor(weights, "model.layers.0.post_attention_layernorm.weight", post_attn_norm_info);
        Tensor mlp_up_proj_tensor = wrap_tensor(weights, "model.layers.0.mlp.up_proj.weight", mlp_up_proj_info);
        Tensor mlp_gate_proj_tensor = wrap_tensor(weights, "model.layers.0.mlp.gate_proj.weight", mlp_gate_proj_info);
        Tensor mlp_down_proj_tensor = wrap_tensor(weights, "model.layers.0.mlp.down_proj.weight", mlp_down_proj_info);
        ...
        Decoder decoder(
            ...
            /*layer_idx*/ 0,
            &cache,
            post_attn_norm_tensor,
            mlp_up_proj_tensor,
            mlp_gate_proj_tensor,
            mlp_down_proj_tensor);

        {
            SectionTimer timer("Decoder::prepare");
            decoder.prepare();
            timer.stop();
        }

        {
            SectionTimer timer("Decoder::run");
            decoder.run(input_tensor, past_sequence_length, output_tensor);
            timer.stop();
        }
...

        {
            ...
            log_success("Input tensor and input flat match after decoder.run");
        }
        ...
        if (match)
        {
            log_success("Decoder output matches golden within tolerance " + std::to_string(kOutputTolerance));
        }
        else
        {
            log_error("Decoder output deviates from golden beyond tolerance " + std::to_string(kOutputTolerance));
            return 2;
        }
    }
